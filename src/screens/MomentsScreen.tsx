    ...
    <Screen scroll>
      ...
      {pending ? (
        <Card style={{ marginBottom: spacing.lg }}>
          <Image source={{ uri: pending }} style={styles.preview} contentFit="cover" transition={250} />
          <Field
            value={caption}
        ...
        <View style={styles.modalBg}>
          ...
          {viewing ? (
            <View style={styles.modalCard}>
              <Image source={{ uri: dataUri(viewing.image) }} style={styles.modalImage} contentFit="contain" transition={200} />
              ...
              return (
                <Pressable key={ci} style={styles.cell} onPress={() => onSelectDay(cell.date!)}>
                  <View style={[styles.cellInner, isSel && styles.cellSelected]}>
                    {has && thumb ? (
                      <Image source={{ uri: dataUri(thumb.image) }} style={styles.cellImage} contentFit="cover" transition={250} />
                    ) : null}
...
const styles = StyleSheet.create({
  ...
  segText: { fontSize: font.size.md, fontFamily: font.family.semibold, color: colors.textSoft },
  segTextActive: { color: colors.text },

  dayHeader: { fontSize: font.size.lg, fontFamily: font.family.displaySemi, color: colors.text, marginTop: spacing.lg, marginBottom: spacing.sm },
  ...
  thumbTagText: { color: colors.white, fontSize: 10, fontFamily: font.family.semibold },

  calHead: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: spacing.md },
  calArrow: { fontSize: 26, color: colors.primary, fontFamily: font.family.bold, width: 36, textAlign: 'center' },
  calTitle: { fontSize: font.size.lg + 1, fontFamily: font.family.displaySemi, color: colors.text },
  weekRow: { flexDirection: 'row' },
  weekday: { flex: 1, textAlign: 'center', fontSize: 11, color: colors.textFaint, fontFamily: font.family.semibold, marginBottom: 4 },
  ...
  cellDay: { fontSize: font.size.sm, color: colors.textSoft, fontFamily: font.family.medium },
  cellDayOnImage: { color: colors.white, fontFamily: font.family.bold, textShadowColor: 'rgba(0,0,0,0.5)', textShadowRadius: 3 },
